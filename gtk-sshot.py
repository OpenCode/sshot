...
def init_db(conn, cr):
    ...
    sql = '%s (id integer primary key default null, name str, host str, port str)' % (sql)
    cr.execute(sql)
    conn.commit()
    return True
...
class Sshot:
    ...
    def __init__(self):
        ...
        self.lst_connections = gtk.ListStore(str, str, str, str)
        self.trv_connections = gtk.TreeView(model=self.lst_connections)
        self.trv_connections.connect('row-activated', self.trv_dbl_click)
    ...
    def trv_dbl_click(self, widget, row, col):
        connection_id = widget.get_model()[row][0]
        print connection_id
        return True
